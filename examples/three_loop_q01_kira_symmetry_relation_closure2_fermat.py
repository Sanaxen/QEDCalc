"""Close the second Q01 symmetry-relation reduction wave with Kira/Fermat.

The first 146-target Fermat supplement closes the numerator-expansion targets,
but the saved relation probe may expose a smaller set of lower-sector integrals
that are still absent from the merged graph.  This helper reads those saved
missing-integral samples from the relation-probe JSON and reduces them with the
ordinary Kira triangular + back-substitution path.  FireFly is deliberately not
used.
"""
...
TARGET_FILE = PROJECT / "q01_symmetry_relation_closure2_fermat_targets"
JOB_FILE = PROJECT / "jobs_q01_symmetry_relation_closure2_fermat.yaml"
LOG_FILE = PROJECT / "q01_symmetry_relation_closure2_fermat.log"
ALT_DIR_NAME = "symmetry_relation_closure2_fermat"
ALT_ROOT = PROJECT / ALT_DIR_NAME
OUTPUT_JSON = PROJECT / "q01_symmetry_relation_closure2_fermat_audit.json"
TOP_SECTOR = 511
...
def _load_targets() -> tuple[IndexTuple, ...]:
    ...
    values = tuple(sorted({_parse(str(x)) for x in raw}))
    return values
...
def generate() -> None:
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-2 Fermat generator")
    print("mode: ordinary triangular + back substitution; FireFly disabled")
    print("closure-wave-2 targets:", len(targets))
    ...
    print("Q01 symmetry relation closure-wave-2 Fermat generation PASS")


def _find_export() -> Path:
    result_dir = ALT_ROOT / "results" / FAMILY
    preferred = result_dir / "kira_q01_symmetry_relation_closure2_fermat_targets.inc"
    if preferred.exists():
        return preferred
    cands = sorted((p for p in result_dir.glob("*.inc") if "closure2_fermat" in p.name), key=lambda p: p.stat().st_mtime, reverse=True)
    if cands:
        return cands[0]
    raise SystemExit(f"ERROR: closure-wave-2 FORM export not found under {result_dir}")
...
def audit() -> None:
    ...
    summary = {
        "mode": "Q01 symmetry relation closure-wave-2 ordinary Kira/Fermat reduction",
        "targets": len(targets),
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-2 Fermat audit")
    ...
    if not unreduced_zero:
        raise SystemExit(1)
    print("Q01 symmetry relation closure-wave-2 Fermat audit PASS")
