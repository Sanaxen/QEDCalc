"""Run Q10/Q50 one-axis Kira seed-boundary master-basis audits."""
...
BASELINE_MASTER_COPY = AUDIT_DIR / "q10_r7s3d0_masters.txt"
BASELINE_AUDIT_JSON = AUDIT_DIR / "three_loop_q10_kira_r7s3d0_audit.json"
SEEDS = {
    "r8s3d0": Q10SeedLimits(8, 3, 0),
    "r7s4d0": Q10SeedLimits(7, 4, 0),
    "r7s3d1": Q10SeedLimits(7, 3, 1),
}


def _project(seed: str) -> Path:
    return ROOT / "output" / f"kira_q10_full_{seed}"


def _audit_json(seed: str) -> Path:
    return AUDIT_DIR / f"three_loop_q10_kira_{seed}_boundary_audit.json"


def _audit_txt(seed: str) -> Path:
    return AUDIT_DIR / f"three_loop_q10_kira_{seed}_boundary_audit.txt"
...
def _load_baseline() -> list[str]:
    if not BASELINE_MASTER_COPY.is_file():
        raise FileNotFoundError(
            ...
            "run run_three_loop_q10_kira_r7s3d0.bat first"
        )
    masters = _parse_master_text(
        BASELINE_MASTER_COPY.read_text(encoding="utf-8", errors="replace")
    )
    if not masters:
        raise ValueError("Q10 baseline master copy contains no parseable masters")
    if BASELINE_AUDIT_JSON.is_file():
        audit = json.loads(BASELINE_AUDIT_JSON.read_text(encoding="utf-8"))
        if not audit.get("audit_pass"):
            raise ValueError("Q10 baseline audit JSON is not PASS")
        if audit.get("master_count") != len(masters):
            raise ValueError(
                f"Q10 baseline count mismatch: audit={audit.get('master_count')} parsed={len(masters)}"
            )
    return masters
...
def prepare(seed: str) -> None:
    ...
    if manifest.get("seed_limits") != expected_limits:
        errors.append(
            f"Q10 {seed} seed manifest mismatch: {manifest.get('seed_limits')} != {expected_limits}"
        )
    if errors:
        for error in errors:
            print("ERROR:", error)
        raise SystemExit(f"QEDCalc Q10 {seed} boundary prepare FAIL")
    ...
    print(f"QEDCalc Q10 {seed} boundary prepare")
    ...
    print(f"QEDCalc Q10 {seed} boundary prepare PASS")
...
def finalize(seed: str) -> None:
    ...
    lines = [
        f"QEDCalc Q10 {seed} seed-boundary audit",
        ...
        f"QEDCalc Q10 {seed} seed-boundary audit " + ("PASS" if not errors else "FAIL"),
    ]
    ...
    if errors:
        for error in errors:
            print("ERROR:", error)
        raise SystemExit(f"QEDCalc Q10 {seed} seed-boundary audit FAIL")
...
def aggregate() -> None:
    ...
    out_json = AUDIT_DIR / "three_loop_q10_kira_seed_boundary_audit.json"
    out_txt = AUDIT_DIR / "three_loop_q10_kira_seed_boundary_audit.txt"
    result = {
        "canonical_family": Q10_KIRA_NAME,
        "covered_diagrams": ["Q10", "Q50"],
        "baseline_seed": {"r": 7, "s": 3, "d": 0},
        ...
        "master_basis_status": "stable_under_tested_one_axis_seed_extensions" if stable else "boundary_review_required",
        "errors": errors,
        "audit_pass": stable,
    }
    out_json.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    lines = [
        "QEDCalc Q10 seed-boundary aggregate audit",
        f"baseline masters: {len(baseline)}",
    ]
    for seed in SEEDS:
    ...
    lines.extend([
        ...
        f"audit TXT: {out_txt}",
        "QEDCalc Q10 seed-boundary aggregate audit " + ("PASS" if stable else "FAIL"),
    ])
    out_txt.write_text("\n".join(lines) + "\n", encoding="utf-8")
    for line in lines:
        print(line)
    if errors:
        for error in errors:
            print("ERROR:", error)
        raise SystemExit("QEDCalc Q10 seed-boundary aggregate audit FAIL")
