...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer10_rank.json"
PRIMES = (1000003, 1000033)
TARGET_COMPLEXITY = 10


def _load_complexity10_indices(base_block):
    ...
    if len(layer) != 1348:
        raise RuntimeError(f"expected 1348 complexity-10 support integrals, got {len(layer)}")
    return layer
...
def _prime_worker(prime: int):
    ...
    if len(base6952) != 6952 or len(target) != 8300:
        raise RuntimeError(
            f"expected base6952/target8300, got {len(base6952)}/{len(target)}"
        )

    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-10 sparse rank start: "
        f"base={len(base6952)}, added={len(layer10)}, target={len(target)}, forbidden={len(forbidden)}",
        flush=True,
    )
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-10 sparse rank done: "
    ...
    return {
        ...
        "base_block_count": len(base6952),
        "added_layer_count": len(layer10),
        ...
        **counts,
        ...
        "added_layer_indices": [index.powers for index in layer10],
    }
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line closure complexity-10 sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; use free17-augmented 8473-seed system; "
        "add only complexity-10 support (1348 integrals)",
        flush=True,
    )
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_layer10_after_free17",
        "runtime": {
    ...
    print(f"added complexity 10: {first['added_layer_count']}")
    ...
    if not stable:
        raise RuntimeError("complexity-10 constrained-rank result differs across primes")
    print("Q01 four-line closure complexity-10 sparse-rank audit PASS")
