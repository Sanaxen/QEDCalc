...
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer15_free3_neighbor_rank import (
    _load_inputs as _load_base14302_inputs,
)

ROOT = Path(__file__).resolve().parents[1]
FREE6_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer14_free_columns.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer14_free6_neighbor_rank.json"
...
def _load_inputs():
    (
        ...
        free15_7,
        free3,
    ) = _load_base14302_inputs()

    free6_data = json.loads(FREE6_SOURCE.read_text(encoding="utf-8"))
    if not free6_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("refreshed complexity-14 free6 basis is not stable across primes")
    rows = free6_data.get("rows", [])
    if not rows:
        raise RuntimeError("refreshed complexity-14 free-column output has no rows")
    free6 = tuple(IntegralIndex(tuple(p)) for p in rows[0]["free_indices"])

    layer14 = _load_refreshed_complexity14_indices(base14302)
    base_set = set(base14302)
    target14595 = tuple(base14302) + tuple(index for index in layer14 if index not in base_set)

    if len(base14302) != 14302 or len(target14595) != 14595 or len(free6) != 6:
        raise RuntimeError(
            f"expected base14302/target14595/free6; got "
            f"{len(base14302)}/{len(target14595)}/{len(free6)}"
        )
...
def _build_augmented_system(prime: int):
    ...
    all_seeds = tuple(
        sorted(base13020 | set(free6_neighbor) | set(free6), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base13020
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base13020={len(base13020)}, refreshed-layer14-free6-neighbors={len(free6_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base13020),
        "free6_neighbor_seed_count": len(free6_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer14 free6-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer14 free6-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-14 free6 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable refreshed complexity-14 free6; "
    ...
    out = {
        ...
        "free_candidate_count": 6,
        "base_seed_count": first["base_seed_count"],
        "free6_neighbor_seed_count": first["free6_neighbor_seed_count"],
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer14_free6_targeted_neighbor",
        "runtime": {
    ...
    print("free candidates targeted: 6")
    print(f"base seeds: {first['base_seed_count']}")
    print(f"free6 neighbor seeds: {first['free6_neighbor_seed_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-14 free6 neighbor rank differs across primes")
    print("Q01 refreshed complexity-14 free6 neighbor sparse-rank audit PASS")
