...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer9_free12_neighbor_rank.json"
...
def _load_inputs():
    ...
    if len(target10283) != 10283 or len(free12_9) != 12:
        raise RuntimeError(
            f"expected target10283/free12; got {len(target10283)}/{len(free12_9)}"
        )
...
def _build_augmented_system(prime: int):
    ...
    all_seeds = tuple(
        sorted(base9681 | set(free12_9_neighbor) | set(free12_9), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base9681
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base9681={len(base9681)}, layer9-free12-neighbors={len(free12_9_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base9681),
        "free12_neighbor_seed_count": len(free12_9_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] layer9 free12-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] layer9 free12-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 closure complexity-9 free12 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable complexity-9 free12; "
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_layer9_free12_targeted_neighbor",
        "runtime": {
    ...
    print("free candidates targeted: 12")
    print(f"base seeds: {first['base_seed_count']}")
    print(f"free12 neighbor seeds: {first['free12_neighbor_seed_count']}")
    ...
    if not stable:
        raise RuntimeError("complexity-9 free12 neighbor constrained-rank result differs across primes")
    print("Q01 closure complexity-9 free12 neighbor sparse-rank audit PASS")
