"""Run the small Q01 symmetry supplement with ordinary Kira/Fermat reduction.

The FireFly reconstruction for the 146 symmetry-derived supplemental targets
fails validation even in fresh alt_dirs and with/without iterative reduction.
This helper therefore isolates only those 146 targets and uses the ordinary
triangular + back-substitution path.  It never modifies the validated
exact944closure1_firefly results.
"""
...
TARGET_FILE = PROJECT / "q01_symmetry_supplement_fermat_targets"
JOB_FILE = PROJECT / "jobs_q01_symmetry_supplement_fermat.yaml"
LOG_FILE = PROJECT / "q01_symmetry_supplement_fermat.log"
ALT_DIR_NAME = "symmetry_supplement_fermat"
ALT_ROOT = PROJECT / ALT_DIR_NAME
OUTPUT_JSON = PROJECT / "q01_symmetry_supplement_fermat_audit.json"
TOP_SECTOR = 511
...
def _load_targets_from_audit() -> tuple[IndexTuple, ...]:
    if not SOURCE_AUDIT.exists():
        raise SystemExit(f"ERROR: source audit not found: {SOURCE_AUDIT}")
    data = json.loads(SOURCE_AUDIT.read_text(encoding="utf-8"))
    raw = data.get("unresolved_targets")
    if not isinstance(raw, list) or not raw:
        raise SystemExit("ERROR: source audit has no unresolved_targets")
    return tuple(sorted({_parse(str(x)) for x in raw}))


def generate() -> None:
    targets = _load_targets_from_audit()
    maxima = [0, 0, 0]
    for v in targets:
        r, s, d = _bounds(v)
    ...
    print("QEDCalc Q01 symmetry supplemental Fermat generator")
    ...
    print("Q01 symmetry supplemental Fermat generation PASS")


def _find_export() -> Path:
    result_dir = ALT_ROOT / "results" / FAMILY
    preferred = result_dir / "kira_q01_symmetry_supplement_fermat_targets.inc"
    if preferred.exists():
        return preferred
    cands = sorted((p for p in result_dir.glob("*.inc") if "symmetry_supplement_fermat" in p.name), key=lambda p: p.stat().st_mtime, reverse=True)
    if cands:
        return cands[0]
    raise SystemExit(f"ERROR: supplemental Fermat FORM export not found under {result_dir}")


def audit() -> None:
    targets = set(_load_targets_from_audit())
    form = _find_export()
    ...
    summary = {
        "mode": "ordinary Kira/Fermat supplemental reduction; FireFly disabled",
        "targets": len(targets),
    ...
    print("QEDCalc Q01 symmetry supplemental Fermat audit")
    ...
    if not unreduced_zero:
        raise SystemExit(1)
    print("Q01 symmetry supplemental Fermat audit PASS")
