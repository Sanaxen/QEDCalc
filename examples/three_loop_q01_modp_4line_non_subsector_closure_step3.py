...
STEP2_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_step2.json"
REDUCTION_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure1850_reduction.json"
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_step3.json"
PRIMES = (1000003, 1000033)
PHYSICAL_COUNT = 9


def main() -> None:
    step2_data = json.loads(STEP2_SOURCE.read_text(encoding="utf-8"))
    ...
    source_sector = tuple(int(x) for x in step2_data["sector"])
    base_block = tuple(IntegralIndex(tuple(p)) for p in step2_data["expanded_block_indices"])

    rows = reduction_data.get("rows", [])
    if len(rows) < 2:
        raise RuntimeError("closure1850 reduction JSON must contain two prime rows")
    ...
    if supports[0] != supports[1]:
        raise RuntimeError("closure1850 RHS support actually differs across primes")
    ...
    result_rows = []
    for prime in PRIMES:
        print(f"[progress] closure step 3 constrained rank prime {prime}", flush=True)
    ...
    print("QEDCalc Q01 four-line non-subsector closure step 3")
    ...
    print("Q01 four-line non-subsector closure step 3 PASS")
