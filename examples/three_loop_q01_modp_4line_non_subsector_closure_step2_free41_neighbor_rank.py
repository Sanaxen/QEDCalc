...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_step2_free41_neighbor_rank.json"
...
def main() -> None:
    ...
    free41_rows = free41_data.get("rows", [])
    if not free41_rows or not free41_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("step2 free-column basis is not stable across primes")
    free41 = tuple(IntegralIndex(tuple(p)) for p in free41_rows[0]["free_indices"])
    ...
    if len(block) != 1850 or len(free41) != 41:
        raise RuntimeError(f"expected 1850-block and 41 free columns; got {len(block)} and {len(free41)}")
    ...
    base_seeds = set(base_seed_layer) | set(free5_neighbor) | set(free5) | set(free8_neighbor) | set(free8)

    free41_neighbor = focused_neighbor_seeds(
        family, free41, templates=templates, physical_count=PHYSICAL_COUNT
    )
    all_seeds = tuple(
        sorted(base_seeds | set(free41_neighbor) | set(free41), key=lambda x: x.powers)
    )
    ...
    rows = []
    for prime in PRIMES:
        print(f"[progress] free41 neighbor constrained rank prime {prime}", flush=True)
    ...
    out = {
        ...
        "free_candidate_count": len(free41),
        "free_candidate_indices": [index.powers for index in free41],
        "base_seed_count": len(base_seeds),
        "free41_neighbor_seed_count": len(free41_neighbor),
    ...
    print("QEDCalc Q01 four-line closure step2 free41 neighbor rank audit")
    print(f"sector: {source_sector}")
    print(f"expanded block: {len(block)}")
    print(f"free candidates: {len(free41)}")
    print(f"base seeds: {len(base_seeds)}")
    print(f"free41 neighbor seeds: {len(free41_neighbor)}")
    ...
    print("Q01 four-line closure step2 free41 neighbor rank audit PASS")
