...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_rank.json"
PRIMES = (1000003, 1000033)
TARGET_COMPLEXITY = 12


def _load_refreshed_complexity12_indices(base_block):
    ...
    if len(layer) != 2546:
        raise RuntimeError(
            f"expected 2546 refreshed complexity-12 support integrals, got {len(layer)}"
        )
    return layer
...
def _prime_worker(prime: int):
    ...
    source_sector, base16151, forbidden, equations, point, setup_timings, counts = (
        _build_augmented_system(prime)
    )
    layer12 = _load_refreshed_complexity12_indices(base16151)
    base_set = set(base16151)
    target = tuple(base16151) + tuple(index for index in layer12 if index not in base_set)
    if len(base16151) != 16151 or len(target) != 18697:
        raise RuntimeError(f"expected base16151/target18697, got {len(base16151)}/{len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-12 sparse rank start: "
        f"base={len(base16151)}, added={len(layer12)}, target={len(target)}, forbidden={len(forbidden)}",
        flush=True,
    )
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-12 sparse rank done: "
    ...
    return {
        ...
        "base_block_count": len(base16151),
        "added_layer_count": len(layer12),
        ...
        **counts,
        ...
        "added_layer_indices": [index.powers for index in layer12],
    }
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-12 sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; use refreshed complexity-13-free22 augmented 15127-seed system; "
        "add only refreshed complexity-12 support (2546 integrals)",
        flush=True,
    )
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer12_after_layer13_free22",
        "runtime": {
    ...
    print(f"added refreshed complexity 12: {first['added_layer_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-12 constrained-rank result differs across primes")
    print("Q01 refreshed complexity-12 sparse-rank audit PASS")
