...
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_free40_neighbor_rank import (
    _build_augmented_system,
)

ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure18697_same_sector_support.json"
...
def _prime_worker(prime: int):
    ...
    if len(block) != 18697:
        raise RuntimeError(f"expected 18697 target integrals, got {len(block)}")
    if len(equations) != 271965:
        raise RuntimeError(f"expected 271965 equations, got {len(equations)}")
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed 18697 support columns ready: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed 18697 sparse support done: "
        f"target_rank={result.target_rank}, free={result.conditional_free_dimension}, "
        f"support={len(result.same_sector_support)}",
        flush=True,
    )

    if result.target_rank != 18697 or result.conditional_free_dimension != 0:
        raise RuntimeError(
            f"expected full-rank 18697 block; got rank={result.target_rank}, "
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line refreshed 18697-block same-sector support audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse same-sector support; no dense inverse; "
        "use refreshed complexity-12-free40 augmented 18131-seed system",
        flush=True,
    )
    ...
    out = {
        ...
        "algorithm": "sparse_refreshed_same_sector_support_after_layer12_free40",
        "runtime": {
    ...
    if not stable:
        raise RuntimeError("refreshed 18697 same-sector support differs across primes")
    print("Q01 refreshed 18697-block same-sector support audit PASS")
