...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer8_free18_neighbor_rank.json"
...
def _load_inputs():
    ...
    if len(target12212) != 12212 or len(free18_8) != 18:
        raise RuntimeError(
            f"expected target12212/free18; got {len(target12212)}/{len(free18_8)}"
        )
...
def _build_augmented_system(prime: int):
    ...
    all_seeds = tuple(
        sorted(base10555 | set(free18_8_neighbor) | set(free18_8), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base10555
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base10555={len(base10555)}, layer8-free18-neighbors={len(free18_8_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base10555),
        "free18_neighbor_seed_count": len(free18_8_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] layer8 free18-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] layer8 free18-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 closure complexity-8 free18 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable complexity-8 free18; "
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_layer8_free18_targeted_neighbor",
        "runtime": {
    ...
    print("free candidates targeted: 18")
    print(f"base seeds: {first['base_seed_count']}")
    print(f"free18 neighbor seeds: {first['free18_neighbor_seed_count']}")
    ...
    if not stable:
        raise RuntimeError("complexity-8 free18 neighbor constrained-rank result differs across primes")
    print("Q01 closure complexity-8 free18 neighbor sparse-rank audit PASS")
