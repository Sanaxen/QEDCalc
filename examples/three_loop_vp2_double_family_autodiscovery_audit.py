"""Run canonical-family autodiscovery for VP4A/VP4B/VP4C and VP22."""
...
OUTPUT_JSON = OUTPUT_DIR / "three_loop_vp2_double_family_autodiscovery_audit.json"
OUTPUT_TXT = OUTPUT_DIR / "three_loop_vp2_double_family_autodiscovery_audit.txt"


def main() -> None:
    rows = load_topologies()
    audit = audit_vp2_double_family_autodiscovery(rows)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    OUTPUT_JSON.write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "QEDCalc VP2/VP22 canonical-family autodiscovery audit",
    ...
    for i, step in enumerate(audit["steps"], start=1):
        lines.append(
            f"  step {i}: rep={step['representative']} status={step['status']} "
            f"family={step['canonical_family_id']} confirmed={step['confirmed_after_step']} "
            f"families={step['family_count_after_step']}"
        )
    ...
    if audit["errors"]:
        for error in audit["errors"]:
            print("ERROR:", error)
        raise SystemExit("QEDCalc VP2/VP22 canonical-family autodiscovery audit FAIL")
