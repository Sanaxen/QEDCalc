...
SOURCE = ROOT / "output" / "3loop_q01_modp_4line_sector_ordered_reduction.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_sector_ordered_rhs_structure.json"
PHYSICAL_COUNT = 9
...
def main() -> None:
    ...
    if not reductions:
        raise RuntimeError("sector-ordered reduction JSON contains no reductions")
    ...
    if any(support != supports[0] for support in supports[1:]):
        raise RuntimeError("sector-ordered RHS support differs across primes")
    ...
    print("QEDCalc Q01 four-line sector-ordered RHS structure")
    ...
    print("Q01 four-line sector-ordered RHS structure PASS")
