...
FREE17_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer14_11_free_columns.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer14_11_free17_neighbor_rank.json"
...
def _load_inputs():
    ...
    free17_data = json.loads(FREE17_SOURCE.read_text(encoding="utf-8"))
    ...
    layer = _load_layer_indices(base5994)
    base_set = set(base5994)
    target6952 = tuple(base5994) + tuple(index for index in layer if index not in base_set)

    if not free57_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("free57 basis is not stable across primes")
    if not free17_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("free17 basis is not stable across primes")
    ...
    free17 = tuple(IntegralIndex(tuple(p)) for p in free17_data["rows"][0]["free_indices"])

    if len(base5994) != 5994 or len(target6952) != 6952 or len(free17) != 17:
        raise RuntimeError(
            f"expected base5994/target6952/free17; got {len(base5994)}/{len(target6952)}/{len(free17)}"
        )
    return source_sector, target6952, free57, free41, free8, free5, dotted, free17


def _build_augmented_system(prime: int):
    source_sector, target, free57, free41, free8, free5, dotted, free17 = _load_inputs()
    ...
    free17_neighbor = focused_neighbor_seeds(
        family, free17, templates=templates, physical_count=PHYSICAL_COUNT
    )
    all_seeds = tuple(
        sorted(base7205 | set(free17_neighbor) | set(free17), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base7205
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base7205={len(base7205)}, free17-neighbors={len(free17_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base7205),
        "free17_neighbor_seed_count": len(free17_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] free17-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] free17-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 closure layer14-11 free17 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable 17 free columns; "
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_free17_targeted_neighbor",
        "runtime": {
    ...
    if not stable:
        raise RuntimeError("free17 neighbor constrained-rank result differs across primes")
    print("Q01 closure layer14-11 free17 neighbor sparse-rank audit PASS")
