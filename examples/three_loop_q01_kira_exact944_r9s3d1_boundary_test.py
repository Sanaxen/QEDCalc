"""Run/audit the Q01 exact944 d+1 seed-boundary extension: r=9,s=3,d=1.
...
bounds, artifact names, and scientific label from r+1 to d+1.
"""
...
# cannot silently change the meaning of this d+1 test. More-specific phrases
...
replacements = [
    ...
    ("planned r10s3d0 seed bounds", "planned r9s3d1 seed bounds"),
    ...
    ("first Q01 exact944 seed-boundary extension", "Q01 exact944 d+1 seed-boundary extension"),
    ("first one-axis stability test beyond the saved r=9,s=3,d=0 scope", "d-axis stability test beyond the saved r=9,s=3,d=0 scope"),
    ("r10s3d0", "r9s3d1"),
]

for old, new in replacements:
    if old not in text:
        raise SystemExit(f"ERROR: expected reference text not found while preparing d+1 helper: {old!r}")
