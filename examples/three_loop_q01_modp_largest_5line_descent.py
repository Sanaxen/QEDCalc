...
OUTPUT = ROOT / "output" / "3loop_q01_modp_largest_5line_descent.json"
TARGET_SECTOR = (1, 0, 1, 0, 1, 1, 0, 1, 0)
PRIME = 1000003


def main() -> None:
    print("QEDCalc Q01 largest 5-line mod-p sector descent")
    data = json.loads(SOURCE.read_text(encoding="utf-8"))
    if "terminal_indices" not in data:
        raise RuntimeError(
            "saved 6-line descent JSON does not contain terminal_indices; "
            "rerun run_three_loop_q01_modp_largest_6line_descent.bat first"
        )

    targets = tuple(
        IntegralIndex(tuple(powers))
        for powers in data["terminal_indices"]
        if physical_sector(IntegralIndex(tuple(powers))) == TARGET_SECTOR
    )
    ...
    print("Q01 largest 5-line mod-p sector descent PASS")
