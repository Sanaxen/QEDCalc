...
ALL_HIGHER_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_all_higher_sector_rank.json"
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_all_higher_sector_reduction.json"
...
def main() -> None:
    ...
    all_higher_data = json.loads(ALL_HIGHER_SOURCE.read_text(encoding="utf-8"))
    ...
    higher = tuple(IntegralIndex(tuple(p)) for p in all_higher_data["higher_indices"])
    ...
    if len(higher) != int(all_higher_data["all_higher_column_count"]):
        raise RuntimeError("all-higher column count does not match audit JSON")
    ...
    for run_no, prime in enumerate(PRIMES, start=1):
        print(
            f"[progress] all-higher sector-ordered reduction prime {prime} "
            f"({run_no}/{len(PRIMES)})",
            flush=True,
        )
        reductions.append(
            reduce_block_sector_ordered_mod_p(probed, block, higher, prime)
        )
        ...
        rows.append(
            {
                ...
                "higher_rhs_count": len(higher_rhs),
                "higher_rhs_indices": [index.powers for index in higher_rhs],
    ...
    stable_higher_zero = all(row["higher_rhs_count"] == 0 for row in rows)
    out = {
        ...
        "all_higher_rhs_zero": stable_higher_zero,
        "rows": rows,
    }
    OUTPUT.write_text(json.dumps(out, indent=2), encoding="utf-8")

    print("QEDCalc Q01 four-line all-higher sector-ordered mod-p reduction")
    ...
    print(f"all higher RHS zero: {stable_higher_zero}")
    for row in rows:
        print(
            ...
            f"higher RHS={row['higher_rhs_count']}, "
            f"rhs term count min/max=({row['rhs_term_count_min']}, {row['rhs_term_count_max']})"
        )
    print(f"generated: {OUTPUT}")
    print("Q01 four-line all-higher sector-ordered mod-p reduction PASS")
