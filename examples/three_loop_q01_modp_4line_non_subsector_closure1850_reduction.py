...
NEIGHBOR_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_step2_free41_neighbor_rank.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure1850_reduction.json"
...
def main() -> None:
    ...
    neighbor_data = json.loads(NEIGHBOR_SOURCE.read_text(encoding="utf-8"))
    ...
    if len(forbidden) != expected_forbidden:
        raise RuntimeError(
            f"forbidden reconstruction mismatch: {len(forbidden)} != {expected_forbidden}"
        )

    reductions = []
    for run_no, prime in enumerate(PRIMES, start=1):
        print(
            f"[progress] closure1850 sector-ordered reduction prime {prime} "
            f"({run_no}/{len(PRIMES)})",
            flush=True,
        )
        ...
        rows.append(
            {
                ...
                "non_subsector_rhs_count": len(non_subsector_rhs),
                "non_subsector_rhs_indices": [index.powers for index in non_subsector_rhs],
    ...
    all_non_subsector_zero = all(row["non_subsector_rhs_count"] == 0 for row in rows)
    out = {
    ...
    print("QEDCalc Q01 four-line closure1850 non-subsector mod-p reduction")
    ...
    print("Q01 four-line closure1850 non-subsector mod-p reduction PASS")
