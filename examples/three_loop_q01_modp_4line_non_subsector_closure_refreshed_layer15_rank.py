...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer15_rank.json"
PRIMES = (1000003, 1000033)
TARGET_COMPLEXITY = 15


def _load_refreshed_complexity15_indices(base_block):
    ...
    if len(layer) != 207:
        raise RuntimeError(f"expected 207 refreshed complexity-15 support integrals, got {len(layer)}")
    return layer


def _prime_worker(prime: int):
    started = time.perf_counter()
    source_sector, base14095, forbidden, equations, point, setup_timings, counts = (
        _build_augmented_system(prime)
    )
    layer15 = _load_refreshed_complexity15_indices(base14095)
    base_set = set(base14095)
    target = tuple(base14095) + tuple(index for index in layer15 if index not in base_set)
    if len(base14095) != 14095 or len(target) != 14302:
        raise RuntimeError(f"expected base14095/target14302, got {len(base14095)}/{len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-15 sparse rank start: "
        f"base={len(base14095)}, added={len(layer15)}, target={len(target)}, forbidden={len(forbidden)}",
        flush=True,
    )
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-15 sparse rank done: "
    ...
    return {
        ...
        "base_block_count": len(base14095),
        "added_layer_count": len(layer15),
        ...
        **counts,
        ...
        "added_layer_indices": [index.powers for index in layer15],
    }
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-15 sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; use refreshed 14095-block / 12773-seed system; "
        "add only refreshed complexity-15 support (207 integrals)",
        flush=True,
    )
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer15_after_layer7_free15",
        "runtime": {
    ...
    print(f"added refreshed complexity 15: {first['added_layer_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-15 constrained-rank result differs across primes")
    print("Q01 refreshed complexity-15 sparse-rank audit PASS")
