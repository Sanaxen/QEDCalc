...
SOURCE = ROOT / "output" / "3loop_q01_modp_4line_mixed18_block_reduction.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_mixed18_rhs_structure.json"
PHYSICAL_COUNT = 9
...
def main() -> None:
    ...
    if not reductions:
        raise RuntimeError("mixed18 reduction JSON contains no reductions")
    ...
    if any(support != support_sets[0] for support in support_sets[1:]):
        raise RuntimeError("mixed18 RHS support differs across primes")

    indices = tuple(IntegralIndex(powers) for powers in sorted(support_sets[0]))
    same = []
    lower = []
    other = []
    ...
    out = {
        ...
        "higher_or_other_indices": [index.powers for index in sorted(other, key=lambda x: x.powers)],
    }
    OUTPUT.write_text(json.dumps(out, indent=2), encoding="utf-8")

    print("QEDCalc Q01 four-line mixed 18-block RHS structure")
    ...
    print(f"higher/other RHS: {len(other)}")
    ...
    print("same-sector residual integrals:")
    for index in sorted(same, key=lambda x: x.powers):
        print(f"  I{index.powers}")
    print(f"generated: {OUTPUT}")
    print("Q01 four-line mixed 18-block RHS structure PASS")
