"""Close the fifth Q01 symmetry-relation reduction wave with Kira/Fermat.

This stage consumes the single remaining lower-sector integral reported by the
wave-4 relation probe and reduces it with ordinary Kira triangular +
...
TARGET_FILE = PROJECT / "q01_symmetry_relation_closure5_fermat_targets"
JOB_FILE = PROJECT / "jobs_q01_symmetry_relation_closure5_fermat.yaml"
LOG_FILE = PROJECT / "q01_symmetry_relation_closure5_fermat.log"
ALT_DIR_NAME = "symmetry_relation_closure5_fermat"
ALT_ROOT = PROJECT / ALT_DIR_NAME
OUTPUT_JSON = PROJECT / "q01_symmetry_relation_closure5_fermat_audit.json"
TOP_SECTOR = 511
...
def _load_targets() -> tuple[IndexTuple, ...]:
    ...
    if missing_occurrences != 1:
        raise SystemExit(
            "ERROR: closure-wave-5 expected exactly one remaining missing occurrence; "
            f"got {missing_occurrences}"
        )
    values = tuple(sorted({_parse(str(x)) for x in raw}))
    if len(values) != 1:
        raise SystemExit(f"ERROR: closure-wave-5 expected one unique target; got {len(values)}")
    return values
...
def generate() -> None:
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-5 Fermat generator")
    print("mode: ordinary triangular + back substitution; FireFly disabled")
    print("closure-wave-5 targets:", len(targets))
    ...
    print("Q01 symmetry relation closure-wave-5 Fermat generation PASS")


def _find_export() -> Path:
    result_dir = ALT_ROOT / "results" / FAMILY
    preferred = result_dir / "kira_q01_symmetry_relation_closure5_fermat_targets.inc"
    if preferred.exists():
        return preferred
    cands = sorted((p for p in result_dir.glob("*.inc") if "closure5_fermat" in p.name), key=lambda p: p.stat().st_mtime, reverse=True)
    if cands:
        return cands[0]
    raise SystemExit(f"ERROR: closure-wave-5 FORM export not found under {result_dir}")
...
def audit() -> None:
    ...
    summary = {
        "mode": "Q01 symmetry relation closure-wave-5 ordinary Kira/Fermat reduction",
        "targets": len(targets),
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-5 Fermat audit")
    ...
    if not unreduced_zero:
        raise SystemExit(1)
    print("Q01 symmetry relation closure-wave-5 Fermat audit PASS")
