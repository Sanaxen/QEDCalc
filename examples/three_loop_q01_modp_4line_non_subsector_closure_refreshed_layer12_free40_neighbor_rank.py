...
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_rank import (
    _load_refreshed_complexity12_indices,
)
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer13_free22_neighbor_rank import (
    _load_inputs as _load_base16151_inputs,
)

ROOT = Path(__file__).resolve().parents[1]
FREE40_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_free_columns.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_free40_neighbor_rank.json"
...
def _load_inputs():
    (
        ...
        free6,
        free22,
    ) = _load_base16151_inputs()

    free40_data = json.loads(FREE40_SOURCE.read_text(encoding="utf-8"))
    if not free40_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("refreshed complexity-12 free40 basis is not stable across primes")
    rows = free40_data.get("rows", [])
    if not rows:
        raise RuntimeError("refreshed complexity-12 free-column output has no rows")
    free40 = tuple(IntegralIndex(tuple(p)) for p in rows[0]["free_indices"])

    layer12 = _load_refreshed_complexity12_indices(base16151)
    base_set = set(base16151)
    target18697 = tuple(base16151) + tuple(index for index in layer12 if index not in base_set)

    if len(base16151) != 16151 or len(target18697) != 18697 or len(free40) != 40:
        raise RuntimeError(
            f"expected base16151/target18697/free40; got "
            f"{len(base16151)}/{len(target18697)}/{len(free40)}"
        )
...
def _build_augmented_system(prime: int):
    ...
    base13020 = base12773 | set(free3_neighbor) | set(free3)
    if len(base13020) != 13020:
        raise RuntimeError(f"expected 13020 pre-refreshed-layer14-free6 seeds, got {len(base13020)}")

    free6_neighbor = focused_neighbor_seeds(
        family, free6, templates=templates, physical_count=PHYSICAL_COUNT
    )
    base13499 = base13020 | set(free6_neighbor) | set(free6)
    if len(base13499) != 13499:
        raise RuntimeError(f"expected 13499 pre-refreshed-layer13-free22 seeds, got {len(base13499)}")

    free22_neighbor = focused_neighbor_seeds(
        family, free22, templates=templates, physical_count=PHYSICAL_COUNT
    )
    base15127 = base13499 | set(free22_neighbor) | set(free22)
    if len(base15127) != 15127:
        raise RuntimeError(f"expected 15127 pre-refreshed-layer12-free40 seeds, got {len(base15127)}")

    free40_neighbor = focused_neighbor_seeds(
        family, free40, templates=templates, physical_count=PHYSICAL_COUNT
    )
    all_seeds = tuple(
        sorted(base15127 | set(free40_neighbor) | set(free40), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base15127
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base15127={len(base15127)}, refreshed-layer12-free40-neighbors={len(free40_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base15127),
        "free40_neighbor_seed_count": len(free40_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer12 free40-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer12 free40-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-12 free40 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable refreshed complexity-12 free40; "
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer12_free40_targeted_neighbor",
        "runtime": {
    ...
    print("free candidates targeted: 40")
    print(f"base seeds: {first['base_seed_count']}")
    print(f"free40 neighbor seeds: {first['free40_neighbor_seed_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-12 free40 neighbor rank differs across primes")
    print("Q01 refreshed complexity-12 free40 neighbor sparse-rank audit PASS")
