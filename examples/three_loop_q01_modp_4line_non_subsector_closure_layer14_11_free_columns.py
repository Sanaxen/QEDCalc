...
from examples.three_loop_q01_modp_4line_non_subsector_closure_layer14_11_rank import (
    _load_layer_indices,
)
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer14_11_free_columns.json"
...
def _prime_worker(prime: int):
    ...
    target = tuple(base_block) + tuple(index for index in layer if index not in base_set)
    if len(target) != 6952:
        raise RuntimeError(f"expected 6952 expanded target block, got {len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] layer14-11 sparse free-column audit start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] layer14-11 sparse free-column audit done: "
        f"target_rank={result.target_rank}, free={result.conditional_free_dimension}",
        flush=True,
    )

    if result.target_rank != 6935 or result.conditional_free_dimension != 17:
        raise RuntimeError(
            f"expected target rank 6935 and free dimension 17; got "
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line closure complexity 14-11 sparse free-column audit")
    ...
    if not stable:
        raise RuntimeError("layer14-11 free-column basis differs across primes")
    print("Q01 four-line closure complexity 14-11 sparse free-column audit PASS")
