...
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer13_rank import (
    _load_refreshed_complexity13_indices,
)
from examples.three_loop_q01_modp_4line_non_subsector_closure_refreshed_layer14_free6_neighbor_rank import (
    _load_inputs as _load_base14595_inputs,
)

ROOT = Path(__file__).resolve().parents[1]
FREE22_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer13_free_columns.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer13_free22_neighbor_rank.json"
...
def _load_inputs():
    (
        ...
        free3,
        free6,
    ) = _load_base14595_inputs()

    free22_data = json.loads(FREE22_SOURCE.read_text(encoding="utf-8"))
    if not free22_data.get("stable_free_basis_across_primes", False):
        raise RuntimeError("refreshed complexity-13 free22 basis is not stable across primes")
    rows = free22_data.get("rows", [])
    if not rows:
        raise RuntimeError("refreshed complexity-13 free-column output has no rows")
    free22 = tuple(IntegralIndex(tuple(p)) for p in rows[0]["free_indices"])

    layer13 = _load_refreshed_complexity13_indices(base14595)
    base_set = set(base14595)
    target16151 = tuple(base14595) + tuple(index for index in layer13 if index not in base_set)

    if len(base14595) != 14595 or len(target16151) != 16151 or len(free22) != 22:
        raise RuntimeError(
            f"expected base14595/target16151/free22; got "
            f"{len(base14595)}/{len(target16151)}/{len(free22)}"
        )
...
def _build_augmented_system(prime: int):
    ...
    all_seeds = tuple(
        sorted(base13499 | set(free22_neighbor) | set(free22), key=lambda index: index.powers)
    )
    added = set(all_seeds) - base13499
    seed_seconds = time.perf_counter() - started

    print(
        f"[worker pid={os.getpid()} prime={prime}] seeds ready: "
        f"base13499={len(base13499)}, refreshed-layer13-free22-neighbors={len(free22_neighbor)}, "
    ...
    counts = {
        "base_seed_count": len(base13499),
        "free22_neighbor_seed_count": len(free22_neighbor),
...
def _prime_worker(prime: int):
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer13 free22-neighbor sparse rank start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed layer13 free22-neighbor sparse rank done: "
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-13 free22 neighbor sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; add neighbor seeds only for the stable refreshed complexity-13 free22; "
    ...
    out = {
        ...
        "free_candidate_count": 22,
        "base_seed_count": first["base_seed_count"],
        "free22_neighbor_seed_count": first["free22_neighbor_seed_count"],
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer13_free22_targeted_neighbor",
        "runtime": {
    ...
    print("free candidates targeted: 22")
    print(f"base seeds: {first['base_seed_count']}")
    print(f"free22 neighbor seeds: {first['free22_neighbor_seed_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-13 free22 neighbor rank differs across primes")
    print("Q01 refreshed complexity-13 free22 neighbor sparse-rank audit PASS")
