...
from examples.three_loop_q01_modp_4line_non_subsector_closure_layer7_rank import (
    _load_complexity7_indices,
)
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer7_free_columns.json"
...
def _prime_worker(prime: int):
    ...
    if len(target) != 14095:
        raise RuntimeError(f"expected 14095 target integrals, got {len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-7 sparse free-column audit start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-7 sparse free-column audit done: "
        f"target_rank={result.target_rank}, free={result.conditional_free_dimension}",
        flush=True,
    )

    if result.target_rank != 14080 or result.conditional_free_dimension != 15:
        raise RuntimeError(
            f"expected target rank 14080 and free dimension 15; got "
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line closure complexity-7 sparse free-column audit")
    ...
    if not stable:
        raise RuntimeError("complexity-7 free-column basis differs across primes")
    print("Q01 four-line closure complexity-7 sparse free-column audit PASS")
