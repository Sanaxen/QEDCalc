...
from examples.three_loop_q01_modp_4line_non_subsector_closure_layer8_free18_neighbor_rank import (
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer7_rank.json"
PRIMES = (1000003, 1000033)
TARGET_COMPLEXITY = 7


def _load_complexity7_indices(base_block):
    ...
    if len(layer) != 1883:
        raise RuntimeError(f"expected 1883 complexity-7 support integrals, got {len(layer)}")
    return layer


def _prime_worker(prime: int):
    started = time.perf_counter()
    source_sector, base12212, forbidden, equations, point, setup_timings, counts = (
        _build_augmented_system(prime)
    )
    layer7 = _load_complexity7_indices(base12212)
    base_set = set(base12212)
    target = tuple(base12212) + tuple(index for index in layer7 if index not in base_set)
    if len(base12212) != 12212 or len(target) != 14095:
        raise RuntimeError(f"expected base12212/target14095, got {len(base12212)}/{len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-7 sparse rank start: "
        f"base={len(base12212)}, added={len(layer7)}, target={len(target)}, "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-7 sparse rank done: "
    ...
    return {
        ...
        "base_block_count": len(base12212),
        "added_layer_count": len(layer7),
        ...
        **counts,
        ...
        "added_layer_indices": [index.powers for index in layer7],
    }
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line closure complexity-7 sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; use complexity-8-free18 augmented 11742-seed system; "
        "add only complexity-7 support (1883 integrals)",
        flush=True,
    )
    ...
    out = {
        ...
        "free18_neighbor_seed_count": first["free18_neighbor_seed_count"],
        ...
        "algorithm": "sparse_forward_constrained_rank_layer7_after_layer8_free18",
        "runtime": {
    ...
    print(f"added complexity 7: {first['added_layer_count']}")
    ...
    if not stable:
        raise RuntimeError("complexity-7 constrained-rank result differs across primes")
    print("Q01 four-line closure complexity-7 sparse-rank audit PASS")
