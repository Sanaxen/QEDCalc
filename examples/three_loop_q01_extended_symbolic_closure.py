...
DOT_SOURCE = ROOT / "output" / "3loop_q01_boundary_dot_symbolic_reduction.json"
OUTPUT = ROOT / "output" / "3loop_q01_extended_symbolic_closure.json"
...
def main() -> None:
    print("QEDCalc Q01 extended exact symbolic dependency closure")
    targets = load_targets(TARGET_SOURCE)
    direct_rules = load_rules(DIRECT_SOURCE)
    reverse_rules = load_rules(REVERSE_SOURCE)
    dot_rules = load_rules(DOT_SOURCE)
    profile = audit_extended_symbolic_closure(
        targets, direct_rules, reverse_rules, dot_rules
    )
    ...
    out = {
        ...
        "dot_boundary_rule_count": profile.dot_boundary_rule_count,
    ...
    print(f"dot-boundary symbolic rules: {profile.dot_boundary_rule_count}")
    print(f"merged exact symbolic rules: {profile.merged_rule_count}")
    print(f"fully closed through extended rules: {closure.fully_closed_target_count}")
    ...
    print("Q01 extended exact symbolic dependency closure PASS")
