...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer12_free_columns.json"
...
def _prime_worker(prime: int):
    ...
    if len(target) != 18697:
        raise RuntimeError(f"expected 18697 target integrals, got {len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-12 sparse free-column audit start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-12 sparse free-column audit done: "
        f"target_rank={result.target_rank}, free={result.conditional_free_dimension}",
        flush=True,
    )

    if result.target_rank != 18657 or result.conditional_free_dimension != 40:
        raise RuntimeError(
            f"expected target rank 18657 and free dimension 40; got "
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-12 sparse free-column audit")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-12 free-column basis differs across primes")
    print("Q01 refreshed complexity-12 sparse free-column audit PASS")
