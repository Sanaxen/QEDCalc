...
RANK48_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_expanded48_rank.json"
...
RHS_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_expanded48_rhs_structure.json"
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_expanded179_rank.json"
PRIMES = (1000003, 1000033)


def main() -> None:
    rank48_data = json.loads(RANK48_SOURCE.read_text(encoding="utf-8"))
    ...
    block48 = tuple(IntegralIndex(tuple(p)) for p in rank48_data["block_indices"])
    ...
    block = tuple(dict.fromkeys(block48 + same_rhs))
    if len(block48) != 48 or len(same_rhs) != 131 or len(block) != 179:
        raise RuntimeError(
            f"expected 48 + 131 = 179 unique same-sector integrals, got "
            f"{len(block48)} + {len(same_rhs)} -> {len(block)}"
        )
    ...
    print("QEDCalc Q01 four-line expanded 179-block rank audit")
    print(f"sector: {tuple(rhs_data['source_sector'])}")
    print(f"base 48-block: {len(block48)}")
    ...
    print(f"restricted 179-block ranks: {restricted_ranks}")
    print(f"conditional free dimensions: {free_dimensions}")
    print(f"stable across primes: {stable}")
    print(f"generated: {OUTPUT}")
    print("Q01 four-line expanded 179-block rank audit PASS")
