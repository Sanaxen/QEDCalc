...
from examples.three_loop_q01_modp_4line_non_subsector_closure_layer9_rank import (
    _load_complexity9_indices,
)
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_layer9_free_columns.json"
...
def _prime_worker(prime: int):
    ...
    if len(target) != 10283:
        raise RuntimeError(f"expected 10283 target integrals, got {len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-9 sparse free-column audit start: "
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] complexity-9 sparse free-column audit done: "
        f"target_rank={result.target_rank}, free={result.conditional_free_dimension}",
        flush=True,
    )

    if result.target_rank != 10271 or result.conditional_free_dimension != 12:
        raise RuntimeError(
            f"expected target rank 10271 and free dimension 12; got "
...
def main() -> None:
    ...
    print("QEDCalc Q01 four-line closure complexity-9 sparse free-column audit")
    ...
    if not stable:
        raise RuntimeError("complexity-9 free-column basis differs across primes")
    print("Q01 four-line closure complexity-9 sparse free-column audit PASS")
