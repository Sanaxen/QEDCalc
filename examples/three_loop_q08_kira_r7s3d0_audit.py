"""Prepare/finalize the Q08/Q48 baseline Kira master-basis run.

This is deliberately only a baseline master-set audit.  A successful r7s3d0
run does not promote Q08_full to master-basis complete; the r+1, s+1, and d+1
boundary seeds must still be compared.
"""
...
PROJECT = ROOT / "output" / "kira_q08_full_r7s3d0"
AUDIT_DIR = ROOT / "output" / "three_loop_integral_family_audit"
AUDIT_JSON = AUDIT_DIR / "three_loop_q08_kira_r7s3d0_audit.json"
AUDIT_TXT = AUDIT_DIR / "three_loop_q08_kira_r7s3d0_audit.txt"
MASTER_COPY = AUDIT_DIR / "q08_r7s3d0_masters.txt"
LIMITS = Q08SeedLimits(r=7, s=3, d=0)
BOUNDARY_SEEDS = ["r8s3d0", "r7s4d0", "r7s3d1"]


def _clean_runtime_outputs() -> list[str]:
    """Remove stale Kira runtime state while preserving generated input files."""
    removed: list[str] = []
    for name in (
        "results",
        "sectormappings",
...
def prepare() -> None:
    ...
    if manifest.get("seed_limits") != {"r": 7, "s": 3, "d": 0}:
        errors.append(f"Q08 baseline seed changed: {manifest.get('seed_limits')}")
    if errors:
        for error in errors:
            print("ERROR:", error)
        raise SystemExit("QEDCalc Q08 r7s3d0 baseline prepare FAIL")
...
def _masters_final_candidates() -> list[Path]:
    return sorted(PROJECT.rglob("masters.final"), key=lambda p: str(p))
...
def _parse_master_integrals(path: Path) -> list[str]:
    ...
    # Keep deterministic first occurrence order while suppressing duplicates.
    masters: list[str] = []
    seen: set[str] = set()
    for item in found:
...
def finalize() -> None:
    errors: list[str] = []
    candidates = _masters_final_candidates()
    if not candidates:
        errors.append(f"no masters.final found under {PROJECT}")
        masters_path = None
        masters: list[str] = []
    else:
        # Kira may leave historical copies in nested directories only if the
        # runtime was not cleaned.  Treat multiple files as an audit error.
        if len(candidates) != 1:
            errors.append(
                "multiple masters.final files found: " + ", ".join(str(path) for path in candidates)
            )
    ...
    audit = {
        ...
        "promotion_rule": (
            "Do not assign a Q08 master_basis_id until r8s3d0, r7s4d0, and r7s3d1 "
            "boundary audits have been compared against this baseline."
        ),
        "errors": errors,
    ...
    lines = [
        "QEDCalc Q08 r7s3d0 baseline Kira audit",
        f"canonical family: {Q08_KIRA_NAME}",
        "covered diagrams: ['Q08', 'Q48']",
        "seed: r7s3d0",
        f"top sector: {Q08_KIRA_TOP_SECTOR}",
        ...
        "QEDCalc Q08 r7s3d0 baseline Kira audit " + ("PASS" if not errors else "FAIL"),
    ]
    ...
    if errors:
        for error in errors:
            print("ERROR:", error)
        raise SystemExit("QEDCalc Q08 r7s3d0 baseline Kira audit FAIL")
