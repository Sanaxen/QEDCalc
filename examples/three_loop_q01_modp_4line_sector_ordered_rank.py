...
HIGHER_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_expanded179_higher_rhs_audit.json"
...
def main() -> None:
    ...
    higher_data = json.loads(HIGHER_SOURCE.read_text(encoding="utf-8"))
    ...
    higher_rows = higher_data.get("higher_integrals") or higher_data.get("higher_or_other_indices")
    if higher_rows is None:
        # The audit runner stores per-integral rows under the first prime when available.
        prime_rows = higher_data.get("prime_rows", [])
        if prime_rows:
            higher_rows = [row["index"] for row in prime_rows[0].get("integrals", [])]
    higher = tuple(IntegralIndex(tuple(p)) for p in higher_rows)
    ...
    stable = all(row == rows[0] | {"prime": row["prime"]} for row in rows[1:]) if rows else True
    # Compare numerical signatures explicitly; prime itself should differ.
    signatures = [
