"""Close the fourth Q01 symmetry-relation reduction wave with Kira/Fermat.

This stage consumes the remaining lower-sector integrals reported by the
wave-3 relation probe and reduces them with ordinary Kira triangular +
...
TARGET_FILE = PROJECT / "q01_symmetry_relation_closure4_fermat_targets"
JOB_FILE = PROJECT / "jobs_q01_symmetry_relation_closure4_fermat.yaml"
LOG_FILE = PROJECT / "q01_symmetry_relation_closure4_fermat.log"
ALT_DIR_NAME = "symmetry_relation_closure4_fermat"
ALT_ROOT = PROJECT / ALT_DIR_NAME
OUTPUT_JSON = PROJECT / "q01_symmetry_relation_closure4_fermat_audit.json"
TOP_SECTOR = 511
...
def generate() -> None:
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-4 Fermat generator")
    print("mode: ordinary triangular + back substitution; FireFly disabled")
    print("closure-wave-4 targets:", len(targets))
    ...
    print("Q01 symmetry relation closure-wave-4 Fermat generation PASS")


def _find_export() -> Path:
    result_dir = ALT_ROOT / "results" / FAMILY
    preferred = result_dir / "kira_q01_symmetry_relation_closure4_fermat_targets.inc"
    if preferred.exists():
        return preferred
    cands = sorted((p for p in result_dir.glob("*.inc") if "closure4_fermat" in p.name), key=lambda p: p.stat().st_mtime, reverse=True)
    if cands:
        return cands[0]
    raise SystemExit(f"ERROR: closure-wave-4 FORM export not found under {result_dir}")
...
def audit() -> None:
    ...
    summary = {
        "mode": "Q01 symmetry relation closure-wave-4 ordinary Kira/Fermat reduction",
        "targets": len(targets),
    ...
    print("QEDCalc Q01 symmetry relation closure-wave-4 Fermat audit")
    ...
    if not unreduced_zero:
        raise SystemExit(1)
    print("Q01 symmetry relation closure-wave-4 Fermat audit PASS")
