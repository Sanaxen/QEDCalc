"""Generate a low-memory FireFly retry for Q01 closure-wave-1.

This reuses the validated closure-wave-1 mandatory target list, but isolates all
new state under a new alt_dir.  It deliberately does not touch exact944closure1.
The job uses FireFly for the full reduction, sectorwise iterative reduction for
lower peak memory, and leaves bunch-size control to the launcher.
"""
...
JOB_FILE = PROJECT / "jobs_exact944_closure1_firefly.yaml"
FAMILY = "Q01_full"
ALT_DIR = "exact944closure1_firefly"
...
def main() -> None:
    print("QEDCalc Q01 exact944 closure-wave-1 FireFly generator")
    print("mode: full FireFly reduction; sectorwise iterative reduction; isolated alt_dir")
    ...
    if not lines or len(lines) != len(set(lines)):
        raise SystemExit(
            "ERROR: closure-wave-1 target list must be non-empty and unique; "
    ...
    if missing:
        raise SystemExit(f"ERROR: generated closure1 FireFly job audit failed: {missing}")
    ...
    print("Q01 exact944 closure-wave-1 FireFly generation PASS")
