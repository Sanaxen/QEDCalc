...
OUTPUT = ROOT / "output" / "3loop_q01_modp_replay_closure_profile.json"
...
def main() -> None:
    print("QEDCalc Q01 mod-p replay dependency closure profile")
    ...
    profile = profile_replay_closure_sizes(trace, sector_targets)
    hist = Counter(record.replay_pivot_count for record in profile.records)
    ...
    out = {
        ...
        "min_replay_pivot_count": profile.min_replay_pivot_count,
        "max_replay_pivot_count": profile.max_replay_pivot_count,
        "replay_pivot_count_histogram": dict(sorted(hist.items())),
    ...
    print(f"min replay pivots per solved target: {profile.min_replay_pivot_count}")
    print(f"max replay pivots per solved target: {profile.max_replay_pivot_count}")
    print(f"replay-pivot histogram: {dict(sorted(hist.items()))}")
    print(f"generated: {OUTPUT}")
    print("Q01 mod-p replay dependency closure profile PASS")
