"""Generate a triangular-only Kira job for the Q01 910-integral demand.

This script does not run Kira.  It validates the existing full-demand project
and writes jobs_triangular.yaml with sectorwise triangular reduction enabled
while back substitution remains disabled.
"""
...
TRIANGULAR_JOB = PROJECT / "jobs_triangular.yaml"
FAMILY = "Q01_full"
EXPECTED_TOP = [511]
EXPECTED_DEMAND = 944
...
def _render_triangular_job() -> str:
    return f'''jobs:
  - reduce_sectors:
      reduce:
...
def main() -> None:
    print("QEDCalc Q01 full-demand Kira triangular job generator")
    ...
    TRIANGULAR_JOB.write_text(_render_triangular_job(), encoding="utf-8", newline="\n")
    text = TRIANGULAR_JOB.read_text(encoding="utf-8")
    required_tokens = [
        "topologies: [Q01_full]",
        "sectors: [511]",
        "r: 9",
    ...
    if missing:
        raise SystemExit(f"ERROR: generated triangular job audit failed: {missing}")

    print("family:", FAMILY)
    print("top sector: 511")
    print("demand baseline: r=9 s=3 d=0")
    print("generated:", TRIANGULAR_JOB)
    print("NOTE: sectorwise triangular reduction is enabled; back substitution is disabled.")
    print("Q01 full-demand Kira triangular job generation PASS")
