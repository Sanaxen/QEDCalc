...
BASE_RANK_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_expanded179_rank.json"
NON_SUBSECTOR_RANK_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_rank.json"
REDUCTION_SOURCE = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_reduction.json"
...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_step1.json"
PRIMES = (1000003, 1000033)
PHYSICAL_COUNT = 9


def main() -> None:
    base_rank = json.loads(BASE_RANK_SOURCE.read_text(encoding="utf-8"))
    non_sub_rank = json.loads(NON_SUBSECTOR_RANK_SOURCE.read_text(encoding="utf-8"))
    ...
    source_sector = tuple(int(x) for x in base_rank["sector"])
    base_block = tuple(IntegralIndex(tuple(p)) for p in base_rank["block_indices"])
    forbidden = tuple(IntegralIndex(tuple(p)) for p in non_sub_rank["forbidden_indices"])

    rows = reduction_data.get("rows", [])
    if not rows:
        raise RuntimeError("non-subsector reduction JSON contains no rows")
    ...
    if any(support != supports[0] for support in supports[1:]):
        raise RuntimeError("non-subsector reduction RHS support differs across primes")
    ...
    result_rows = []
    for prime in PRIMES:
        print(f"[progress] closure step 1 constrained rank prime {prime}", flush=True)
    ...
    print("QEDCalc Q01 four-line non-subsector closure step 1")
    ...
    print("Q01 four-line non-subsector closure step 1 PASS")
