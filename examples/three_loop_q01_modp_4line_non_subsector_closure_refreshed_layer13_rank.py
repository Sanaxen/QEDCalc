...
OUTPUT = ROOT / "output" / "3loop_q01_modp_4line_non_subsector_closure_refreshed_layer13_rank.json"
PRIMES = (1000003, 1000033)
TARGET_COMPLEXITY = 13


def _load_refreshed_complexity13_indices(base_block):
    ...
    if len(layer) != 1556:
        raise RuntimeError(
            f"expected 1556 refreshed complexity-13 support integrals, got {len(layer)}"
        )
    return layer
...
def _prime_worker(prime: int):
    ...
    source_sector, base14595, forbidden, equations, point, setup_timings, counts = (
        _build_augmented_system(prime)
    )
    layer13 = _load_refreshed_complexity13_indices(base14595)
    base_set = set(base14595)
    target = tuple(base14595) + tuple(index for index in layer13 if index not in base_set)
    if len(base14595) != 14595 or len(target) != 16151:
        raise RuntimeError(f"expected base14595/target16151, got {len(base14595)}/{len(target)}")

    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-13 sparse rank start: "
        f"base={len(base14595)}, added={len(layer13)}, target={len(target)}, forbidden={len(forbidden)}",
        flush=True,
    )
    ...
    print(
        f"[worker pid={os.getpid()} prime={prime}] refreshed complexity-13 sparse rank done: "
    ...
    return {
        ...
        "base_block_count": len(base14595),
        "added_layer_count": len(layer13),
        ...
        **counts,
        ...
        "added_layer_indices": [index.powers for index in layer13],
    }
...
def main() -> None:
    ...
    print("QEDCalc Q01 refreshed complexity-13 sparse-rank audit")
    print(format_runtime_config(config), flush=True)
    print(
        "memory mode: sparse rank; use refreshed complexity-14-free6 augmented 13499-seed system; "
        "add only refreshed complexity-13 support (1556 integrals)",
        flush=True,
    )
    ...
    out = {
        ...
        "algorithm": "sparse_forward_constrained_rank_refreshed_layer13_after_layer14_free6",
        "runtime": {
    ...
    print(f"added refreshed complexity 13: {first['added_layer_count']}")
    ...
    if not stable:
        raise RuntimeError("refreshed complexity-13 constrained-rank result differs across primes")
    print("Q01 refreshed complexity-13 sparse-rank audit PASS")
