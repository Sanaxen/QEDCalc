...
rem The Fermat binary is known from the WSL setup to be:
rem   $HOME/fermat/Ferl7/fer64
rem Use the executable itself, not the containing Ferl7 directory.
wsl.exe bash -lc "test -x $HOME/fermat/Ferl7/fer64 || { echo ERROR: Fermat binary is missing or not executable: $HOME/fermat/Ferl7/fer64; ls -l $HOME/fermat/Ferl7/fer64 2>/dev/null || true; exit 3; }; echo Fermat: $HOME/fermat/Ferl7/fer64"
if errorlevel 1 (
...
rem Pass the Windows project path directly to WSL to preserve non-ASCII path
rem names.  Set FERMATPATH in the same Bash process that launches Kira.
wsl.exe --cd "%PROJECT_WIN%" bash -lc "set -o pipefail; export FERMATPATH=$HOME/fermat/Ferl7/fer64; echo Fermat: $FERMATPATH; kira jobs_preflight.yaml 2>&1 | tee q01_full_preflight.log"
set "RC=%ERRORLEVEL%"
