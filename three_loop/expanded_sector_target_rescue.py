"""Finite-field Q01 target rescue with one extra same-sector predecessor-seed layer."""
...
class ExpandedSectorRow:
    sector: tuple[int, ...]
    target_count: int
    blocker_seed_count: int
    predecessor_seed_count: int
...
@dataclass(frozen=True)
class ExpandedSectorTargetRescueProfile:
...
def audit_expanded_sector_target_rescue(
    ...
    *,
...
) -> ExpandedSectorTargetRescueProfile:
    ...
    all_predecessors: set[IntegralIndex] = set()
    ...
    for sector_no, sector in enumerate(sectors, start=1):
        ...
        all_blockers.update(blocker_seeds)
        all_predecessors.update(predecessor_seeds)
        all_seeds.update(seeds)
        _progress(
            progress,
            f"sector {sector_no}/{len(sectors)} start: targets={len(sector_targets)} blockers={len(blocker_seeds)} predecessors={len(predecessor_seeds)} seeds={len(seeds)}",
        )
        ...
        rows.append(ExpandedSectorRow(
            sector=sector,
            target_count=len(sector_targets),
            blocker_seed_count=len(blocker_seeds),
            predecessor_seed_count=len(predecessor_seeds),
