...
set "STAMP=%DS%_%TS: =0%"
set "OUT=test_results\v059_%STAMP%"
mkdir "%OUT%" >nul 2>&1
set EXITCODE=0

echo [1/3] Phase 38 raw inner vertex...
"%PY%" examples\phase38_corner_raw_inner_vertex_bridge.py > "%OUT%\phase38.log" 2>&1
set E=!ERRORLEVEL!
type "%OUT%\phase38.log"
if not "!E!"=="0" set EXITCODE=!E!

echo [2/3] Corner regression...
...
if "!EXITCODE!"=="0" (echo v0.59 validation PASS) else (echo v0.59 validation FAIL exit=!EXITCODE!)
> "%OUT%\summary.txt" echo Exit code: !EXITCODE!
>> "%OUT%\summary.txt" echo Expected version: 0.59.0
