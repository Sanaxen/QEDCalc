...
".venv\Scripts\python.exe" -m examples.three_loop_q02_kira_r8s3d0_audit --prepare
if errorlevel 1 exit /b %ERRORLEVEL%

set "WIN_PROJECT=%CD%\output\kira_q02_full_r8s3d0"
if not exist "%WIN_PROJECT%" (
  echo ERROR: Q02 Kira project was not found.
  echo Expected: %WIN_PROJECT%
  exit /b 2
)

echo QEDCalc Q02 r8s3d0 Kira run
...
".venv\Scripts\python.exe" -m examples.three_loop_q02_kira_r8s3d0_audit --finalize
