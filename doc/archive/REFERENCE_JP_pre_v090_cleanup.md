...
## 52. v0.52.0：vacuum polarization の raw-to-final と self-energy の解析 downstream

### 52.1 vacuum polarization

raw の閉じた電子ループ trace を shift し、D次元 tensor reduction と scalar loop IBP を用いて `(k^2 g^{alpha beta}-k^alpha k^beta)` に比例する横波構造を自動再生成します。on-shell subtraction は `D->4` より先に差として実行し、有限な対数形 `hat(Pi)_R(k^2)` を dimensional difference から生成します。

その後、outer magnetic kernel、z積分、x原始関数、両端点を順に再生成し、最終係数 `119/36-pi^2/3` まで到達します。既知の最終値は最後の回帰比較にしか使用しません。

実行：`run_phase27_vacuum_polarization_raw_to_final.bat`

### 52.2 self-energy insertion

既に得られている有限4変数 kernel `G_A` から、`Y=ab+q^2 z(1-a)` によって b積分を再生成します。z積分は elementary な `I0(c), I1(c)`、q積分は実区間の対数表示を使い、保存済み1変数 kernel を入力せず同じ `F(a)` を再構成します。

最後のa積分は一般的な power-log 積分、zeta(2) 和、endpoint-safe な telescoping pair から組み立て、有限部 `-1/24-pi^2/18` を再生成します。IR側も独立に `A^(1)->1/2`, `J(rho)=-log(rho)-1/2+o(1)` を導き、2図の和 `log(rho)+11/24-pi^2/18+o(1)` を得ます。

実行：`run_phase28_self_energy_analytic_downstream.bat`

self-energy で残る大きな未接続部は、raw左右2図＋magnetic projector から `G_A` を生成する機械代数部分です。
...
`corner_gaussian_bare_templates()` は v0.55 の raw magnetic-projector 多項式を monomial streaming で平方完成し、odd moment 除去、rank-2/rank-4 tensor reduction、2-loop radial master integral を順に適用して `G4`, `G5` の compact template を生成する。`corner_uv_residue_sample()` は exact-rational UV chart で bare residue を検査し、`corner_local_uv_subtractions()` / `corner_uv_subtracted_residue_sample()` は local `B gamma_rho` subtraction と UV logarithmic residue の exact cancellation を検証する。local subtraction は境界監査用であり、既存の物理的 UV-finite representation の代用品ではない。


## v0.57.0 corner renormalized-inner-vertex sector bridge

`corner_renormalized_inner_vertex_sectors()` keeps the physical Eq. (32)-type remainder in three sectors. `corner_inner_vertex_z_integral_residual()` proves the z-parameter sector closes to `log(Lambda_prime_sq/Lambda0_sq)`. `corner_inner_vertex_kappa_difference_residual()` proves the corrected kappa-squared term uses `(1/Lambda0_sq - 1/Lambda_prime_sq)`. These functions are deliberately separate from the local five-simplex UV subtraction, which remains a boundary diagnostic only.

## 66. v0.58.0: corner Lambda-prime -> outer quadratic-form bridge

`corner_lambda_prime_bulk_coefficients()` constructs the rho=0 momentum dependence of the renormalized inner denominator from
...
with $D(k)=k^2-2p\cdot k$ and $m=1$. Thus the generated coefficients are $uv(1-uv)$ for $k^2$ and $uv(1-u)$ for the linear $p\cdot k$ structure.

Combining the outer photon, outer electron, and inner denominator with simplex weights $a_p,a_d,a_l$ ($a_p=1-a_d-a_l$) gives
...
and square completion generates

$$
Q=B^2+a_lu^2H.
$$

The z-interpolated inner denominator similarly generates $H_z,B_z,Q_z$. `corner_outer_quadratic_checkpoint_residuals()` verifies exact zero difference from the independently archived downstream polynomials. Run `run_phase37_corner_outer_quadratic_bridge.bat`.

## v0.59.0: raw inner one-loop vertex finite remainder

The corner calculation no longer requires the historical long-form K_nu numerator as an input.  In the rest-frame convention used by the Breit projector, the raw inner one-loop numerator is shifted with x=u v and y=u(1-v).  After symmetric integration over the inner loop momentum r it has the exact form

$$
r^2\gamma_\nu + C_\nu(k;u,v).
$$

The coefficient of r^2 is exactly gamma_nu for nu=0,1,2,3.  Therefore the dimensional UV pole is purely proportional to gamma_nu.  The complete on-shell difference is generated as
...
-\frac{C_\nu(k;u,v)}{2\Lambda'^2}
+\frac{C_\nu(0;u,v)}{2\Lambda_0^2},
$$

with the overall convention chosen so that the logarithmic sector matches the physical corner convention.  At k=0 the full matrix remainder vanishes exactly.  This bridge supplies the renormalized inner vertex directly to the future outer magnetic-projector stage and avoids reconstructing K_nu from a historical transcription.

## v0.60.0: finite inner remainder to outer Breit projector streams
`corner_outer_projector_streams()` inserts the v0.59 raw-generated finite inner vertex into the right corner outer chain and performs the Breit magnetic projection before outer momentum integration.  To avoid monolithic matrix simplification, the result is kept in three independent scalar streams: the universal logarithmic gamma term, the `-C(k)/(2 Lambda_prime^2)` term, and the `+C(0)/(2 Lambda0^2)` subtraction term. Their base/transverse polynomial counts are 4/2, 21/15, and 6/2.

`corner_outer_stream_denominators()` independently builds the completed finite-rho outer quadratic forms. At rho=0 the Lambda-prime and z families reproduce the previously derived `H,Q,H_z,Q_z` exactly; the Lambda0 stream reduces to the two-denominator outer family with `Q0=a_d^2+(1-a_d)rho^2`.

## v0.62.0: corner inner-loop D-dimensional audit

The UV-divergent rank-two inner numerator must retain D-dimensional gamma algebra until after the pole expansion.  Symmetric D-dimensional reduction gives
`c(D)=(D-2)^2/D` multiplying `gamma_nu r^2`.  For `D=4-2 epsilon`, QEDCalc generates `1-3 epsilon/2+O(epsilon^2)`.  Multiplication by the normalized UV master `(D/4) Gamma(epsilon)` yields the exact evanescent finite difference `-3/2` relative to a four-dimensional-first reduction.  This is an intermediate local audit, not a physical corner correction by itself.

### Phase 44: evanescent local term and on-shell subtraction

The D-dimensional local finite term generated in Phase 43 is proportional to `gamma_nu`. Since the on-shell constant `B` is defined from the same local gamma channel, the identical contribution is present in both the bare vertex and `B gamma_nu`. QEDCalc verifies symbolically that their difference vanishes before outer integration and that both outer Breit-projector scalar streams also cancel exactly.


## v0.65.0: inner/outer photon sign ownership

The sequential corner route contains two Feynman-gauge photon numerator signs.  The raw inner Dirac chain used to generate the renormalized inner remainder omits the explicit inner `-g_{alpha beta}` sign, and the compact outer chain omits the explicit remaining outer `-g_{rho sigma}` sign.  Their product is `+1`, matching the original bare two-photon corner sign.  An outer-only local-gamma insertion needs the outer minus sign to reproduce the Schwinger density, but that one-sign-only correction must not be applied to the full corner.  Phase 46 records this ownership explicitly and leaves the full physical kernel sign unchanged.


## 67. v0.67.0：corner historical $K_\nu$ full-chain projector の独立再生成

### 67.1 目的

v0.61 以降の physical LP route は raw inner numerator から生成した $C_\nu(k;u,v)$ を使用している。一方、独立導出資料には Eq. (31) を on-shell 化した $K_\nu$ operator と、full magnetic projector 後の構造

$$
P_K=D(k)Q_K+R_{\mathrm{odd}}
$$

が保存されている。Phase 49 は、既知の $Q_K$ や最終 finite parameter kernel を入力せず、保存されている operator だけからこの構造を再生成する監査層である。

### 67.2 current convention へ転記する operator

右側 external Dirac equation で消える項を除いた $K_\nu$ は、

$$
\begin{aligned}
K_\nu &={}K_\nu^{\mathrm{pres}}+(1-u)(1-uv)D(k)\gamma_\nu \\
K_\nu &\quad {}+2k^2\left[1-u+u^2v(1-v)\right]\gamma_\nu
\end{aligned}
$$

である。denominator-preserving part は、

$$
\begin{aligned}
K_\nu^{\mathrm{pres}} &={}-\left[i\gamma(p-k)+1\right](1-u^2)\gamma_\nu \\
K_\nu^{\mathrm{pres}} &\quad {}-\left[i\gamma(p-k)+1\right]i(1-u)(1-uv)(2p-k)_\nu \\
K_\nu^{\mathrm{pres}} &\quad {}-\left[i\gamma(p-k)+1\right]i(1-u+2uv)(1-uv)k_\nu \\
K_\nu^{\mathrm{pres}} &\quad {}+ik_\nu u(1+u)(1-2v)-\sigma_{\nu\lambda}k^\lambda u(1-u)
\end{aligned}
$$

である。QEDCalc では既存導出で監査済みの convention map、

$$
i\gamma_{\mathrm{KK}}\cdot a\longleftrightarrow-\rlap{/}a
$$

を使用し、current Minkowski 側では
...
`corner_historical_K_projector_audit()` は次を返す。

- `K_pres_matrices`：$K_\nu^{\mathrm{pres}}$ の4成分
- `K_D_matrices`：$(1-u)(1-uv)D(k)\gamma_\nu$
- `K_k2_matrices`：$2k^2[1-u+u^2v(1-v)]\gamma_\nu$
- `K_full_matrices`：上記3 sector の和
- `base`：$2iM_2^{(1)}-M_0^{(0)}$ の numerator
- `transverse`：$M_2^{(0)}$
- `common_numerator`：outer $D_1(q)^{-1}$ の $q$ 微分まで共通分母へ揃えた numerator
- `Q_K`：$D(k)$ で polynomial division した quotient
- `odd_remainder`：division remainder

### 67.4 exact regression

Phase 49 では、

$$
\boxed{
P_K-D(k)Q_K-R_{\mathrm{odd}}=0
}
$$
...
と因数分解される。outer square completion の shift は time component にしか入らないため、$k_1,k_2$ は unshifted transverse variables である。したがって、

$$
\int d^4K_E\,K_1K_2 f(K_E^2)=0
$$
...
### 67.5 v0.61+ LP route との境界

Phase 49 の `Q_K` は独立に保存された $K_\nu$ operator から再生成した監査対象である。`corner_physical_common_quotients().lp_quotient` は raw inner numerator の $C_\nu$ から生成された別 route の object である。

したがって現段階では、

$$
\boxed{
Q_K^{\mathrm{historical}}\neq\text{自動的に }Q_{\mathrm{LP}}^{(C_\nu)}
}
$$

として扱う。両者を同一視した denominator-power の手修正は禁止する。次段では operator level で両 route の差を denominator-preserving、$D(k)\gamma_\nu$、$k^2\gamma_\nu$ sectorへ分解し、同一 convention 下で対応を確定する。

実行：`run_phase49_corner_historical_K_projector_audit.bat`


## 68. v0.68.0 Phase 50：$K_\nu$ の KK/current 規約解決

### 68.1 Phase 49 の位置づけ

Phase 49 の `corner_historical_K_projector_audit()` は、保存資料にある

$$
P_K=D(k)Q_K+R_{\mathrm{odd}}
$$

および $R_{\mathrm{odd}}\propto k_1k_2$ を再生成する structural audit である。この条件だけでは、Karplus--Kroll の explicit $i$、$\sigma_{\nu\lambda}$、および $\overline{\Lambda}$ の符号再定義を一意に決めない。

Karplus--Kroll 原記法と current notation の間では、

$$
i\gamma_{\mathrm{KK}}p_{\mathrm{KK}}\longleftrightarrow-\rlap{/}p
$$

に加え、renormalized remainder について、

$$
\overline{\Lambda}_{\mathrm{current}}=-\overline{\Lambda}_{\mathrm{KK}}
$$

という convention boundary を保持する。

### 68.2 tensor-basis coefficient の自動解決

`corner_K_convention_resolved_audit()` は historical $K_\nu$ を7個の独立 basis に分解し、raw $C_\nu$ から生成される current magnetic projector と一致する係数を解く。

canonical coefficient tuple は、
...
であり、base、transverse、common numerator のすべてで exact に消えるため、canonical coefficient として0を選ぶ。

解決後は、

$$
\boxed{K_\nu^{\mathrm{current}}=\frac12C_\nu-2f(u)\gamma_\nu}
$$

に対応する projector stream と一致し、

$$
\boxed{
P_K^{\mathrm{mapped}}-P_K^{(C)}=0
}
$$

を base、transverse、common numerator で exact に確認する。

### 68.3 API

- `corner_K_convention_resolved_audit()`
- `corner_phase50_K_convention_resolved_audit()`
- `examples/phase50_corner_K_convention_resolved_audit.py`

Phase 50 の regression target は、`base_residual=0`、`transverse_residual=0`、`common_residual=0`、projector-null basis の exact zero、mapped odd remainder の transverse-odd factorization である。

## 69. v0.68.0 Phase 51：rational remainder の regrouping

current rational remainder は、
...
である。Phase 50 の convention-resolved $K_\nu$ を用いると、
...
`corner_rational_regrouping_audit()` は、この identity を4つの inner Lorentz componentで exact に確認する。さらに、common quotient と odd remainder の線形性を使い、$K+\kappa^2$ route と current `LP+B_gamma` route が Gaussian integration 後も一致することを検証する。

Phase 51 の exact residual は、

```text
quotient_residual            0
remainder_residual           0
lp_gaussian_template_residual 0
B_L0_template_residual       0
operator_identity_residuals  (0, 0, 0, 0)
```

である。

したがって、corner finite-part discrepancy の原因候補から rational LP/$K$ sector の operator convention と Gaussian regrouping を除外する。次の監査対象は log sector の direct reconstruction である。

### 69.1 API

- `corner_rational_regrouping_audit()`
- `corner_phase51_rational_regrouping_audit()`
- `examples/phase51_corner_rational_regrouping_audit.py`


## 70. v0.69.0 Phase 52：log sector の unsplit 直接再構成

### 70.1 出発式

log sector は、分割後の rational familyを出発点にせず、
...
### 70.2 denominator cancellation 前の family

outer photon denominator を $P$、outer electron denominator を $E$、interpolated inner denominator を $L_z$ とすれば、分割前の scalar family は

$$
\frac{\left[A_Kk^2+A_DD(k)\right]Q_{\log}(k)}{P E^2L_z}
$$

である。Phase 52 はこの式を4-factor familyのまま Feynman parameter化し、$n=4$ Gaussian masterへ直接渡す。
...
また v0.66 で導入した3-family splitとの元の scalar identityは exact residual 0 となる。direct Gaussian templateにも $\Gamma(0)$ poleは残らない。

この検証により、log sector は「分割後の式だけ」ではなく、分割前の $\log(\Lambda'^2/\Lambda_0^2)$ routeからも独立に生成できるようになった。

### 70.4 数値 cross-check の位置づけ

uniform scrambled Sobolを用いた補助計算では、$\rho=0.05$ において direct unsplit と3-family split の log-sector integralはいずれも約 $0.32$ へ収束する。しかし soft endpoint の分散がまだ大きいため、この数値は回帰テストの基準値には採用しない。

Phase 53 では同一の soft importance mapを両routeへ適用し、direct-split差を統計誤差内まで縮める。


## 71. v0.69.0 Phase 53：soft importance map と compact 数値経路

Phase 53 は、soft scaling $u=O(\rho)$、$a_d,a_p=O(u)$ を重点化しながら全domainを覆う bijectionを実装する。
...
simplex sum、line-map sum、$u(0)=0$、$u(1)=1$、Jacobianを exact に監査する。

数値計算では、展開済みの巨大 rational expressionではなく、`corner_physical_gaussian_templates()` と `corner_log_unsplit_audit().direct_template` を直接評価する。これによりsoft endpointでの浮動小数桁落ちを避ける。

補助的scrambled-Sobol計算では $\rho=0.1,0.05,0.02$ で direct log と split log の差が推定統計誤差内へ入る。この数値結果は診断用であり、pytestの固定基準値にはしない。

## 72. v0.69.0 Phase 54：$B$ finite normalization の独立導出

$D=4-2\epsilon$ で normalized $k^2$ radial masterを展開すると、有限部に

$$
J_{k^2,\mathrm{fin}}=-\ln L-\frac12
$$

が生じる。scalar masterを合わせると、$B$ の finite integrandは
...
$rational$ 部分 $u f(u)/L$ を $L$ で多項式除算すると、hard constantは $-5/4$ となる。また、

$$
I_{\log}=-\int_0^1u\ln(u^2)\,du=\frac12
$$

であり、explicit $-1/2$ termの $u$ 積分は $-1/4$ である。したがって、
...
1-loop magnetic coefficient $A^{(1)}(0)=1/2$ から、
...
この $-11/8$ は final corner constantから逆算した値ではない。current Eq. (32) kernelへ手動加算することは禁止し、次段で subtraction normalizationのownershipを監査する。


## Phase 55: local finite normalization ownership

Phase 54 で得た $B_{\rm fin}$ の local finite pieces を、bare on-shell charge channel と $B\gamma_\nu$ subtraction channel に分けて監査する。

各 $u$ での local coefficient は、

$$
C_{\rm local}(u,\rho)
= -\ln L_0-\frac12-\frac{2f(u)}{L_0}
$$
...
on-shell $B\gamma_\nu$ subtraction は同じ local charge channel から定義されるため、bare側と subtraction側について、

$$
R_{\log}=C_{\log}^{\rm bare}-C_{\log}^{B}=0
$$

$$
R_{1/2}=C_{1/2}^{\rm bare}-C_{1/2}^{B}=0
$$

$$
R_{\rm rat}=C_{\rm rat}^{\rm bare}-C_{\rm rat}^{B}=0
$$
...
である。Phase 54 の counterterm-side finite constantは $-11/8$ だが、bare local channelには対応する $+11/8$ が存在し、renormalized inner remainderへ追加すべき独立項ではない。

さらに current Eq. (32) bridge の $k=0$ temporal charge matrix element、および outer magnetic projector後の base/transverse residualもすべて0である。
...
## Phase 56: sequential normalization と integration measure ownership

Eq. (42) の $1/4$ normalizationを事実として置くのではなく、current Feynman-rule prefactorから再導出する。

片側の sequential insertion の magnitude は、

$$
C_{\rm one}
=\frac{e^2}{(2\pi)^4}\frac{\alpha}{2\pi}
$$

である。$e^2=4\pi\alpha$ を用いると、

$$
\boxed{C_{\rm one}=\frac{\alpha^2}{8\pi^4}}
$$

となる。mirror pair を含めると、

$$
\boxed{C_{\rm pair}=\frac{\alpha^2}{4\pi^4}}
$$

であり、Eq. (42) の amputated prefactorと exact に一致する。outer loop integration の共通 $\pi^2$ を取り出すと、

$$
C_{\rm phys}
=\frac14\left(\frac{\alpha}{\pi}\right)^2
$$

となるため、current physical factor $1/4$ の residualは0である。

inner photon と outer photon の Feynman-gauge numerator signはそれぞれ $-1$ であり、full cornerでは積が $+1$ となる。

また `corner_physical_parameter_kernels()` の返す各 kernel は外側の $u\,du$ measureを含まない。各sectorの積分measureは次のとおりである。

- `lp`: $u\,du\,dv\,da_d\,da_l$
- `B_gamma`: $u\,du\,da_d$
- `log_photon_cancel`: $u\,du\,dv\,dz\,da_l$
- `log_electron_cancel`: $u\,du\,dv\,dz\,da_p\,da_d$
- `log_photon_mass_residual`: $u\,du\,dv\,dz\,da_p\,da_d$
...
Phase 55--56 により、local finite normalization、mirror factor、overall $1/4$ normalization は有限部ずれの原因候補から外れる。次の監査対象は soft-region の非一様 overlap と full-corner QMC convergence である。

## 75. Phase 57：$a_d=ur$ large-$r$ overlap の exact audit

独立導出で得られていた joint soft kernel をQEDCalc本体から再生成する。

$$
D_r = a_l^2v^2-a_l^2+2a_lrv+a_l+r^2
$$

とすると、$K$ sector と $\kappa^2$ sector の和について、
...
```python
corner_large_r_overlap_audit()
corner_phase57_large_r_overlap_audit()
```

数値計算用には、同じ $1/r$ tail を持つ smooth profile、
...
$$
\lim_{r\to\infty} r\left[\mathcal K_K+\mathcal K_{\kappa^2}-\mathcal O(r)\right] = 0
$$

である。この profile の有限部分を単独で physical corner へ加えてはならない。常に同じ積分domainで subtract / add-back を対にして使用する。

## 76. Phase 58：simplex cutoff ownership

$a_d=ur$ かつ $a_p=1-a_l-a_d\ge0$ なので、large-$r$ variable の実際の上限は、
...
Phase 57 の smooth overlap をこの上限まで積分すると、

$$
I_{\rm ov} = \frac{4v}{(1-a_l)^2}\ln\left(1+r_{\max}^2\right)
$$

となる。したがって、

$$
\boxed{\lim_{u\to0} \left[-u\frac{dI_{\rm ov}}{du}\right] = \frac{8v}{(1-a_l)^2}}
$$

であり、large-$r$ overlap が $\ln(1/u)$ を所有することを exact に確認できる。

API：

```python
corner_large_r_cutoff_audit()
corner_phase58_large_r_cutoff_audit()
```

固定 $a_l$ で残る有限項は profile / matching bookkeeping に依存するため、独立な物理補正として使用しない。次段では有限 $\rho$ の compact full-corner kernelに対して同じ cutoff ownershipを持つ add-subtract pairを実装する。

## 77. Phase 59：同一 simplex cutoff 上の overlap add-subtract

Phase 57 の joint soft kernel に対して、Phase 58 で確定した実際の simplex cutoff をそのまま使用する。
...
smooth overlap profileを $\mathcal O(r)$ とすると、QEDCalc は同じdomain上で、

$$
\mathcal K_{\rm joint}=\left(\mathcal K_{\rm joint}-\mathcal O\right)+\mathcal O
$$

を exact に保持する。pointwise recombination residual、analytic add-back residual、subtracted kernel の $1/r$ coefficient はすべて0である。

API：

```python
corner_overlap_add_subtract_audit()
corner_phase59_overlap_add_subtract_audit()
```

この add-back は数値安定化のための恒等変形であり、独立な物理補正ではない。

## 78. Phase 60：measure込み joint soft density の規格化

primary soft scaling は、
...
である。物理measureとJacobianを含む leading density は、
...
$$
\int_0^\infty dS\,G(R,S,v)=\frac{2Rv}{(R+v)^3}
$$
...
```python
corner_joint_soft_density_audit()
corner_phase60_joint_soft_density_audit()
```

この結果により、universal IR logarithm の所有者は単独の sequential family ではなく、measure込み joint soft density であることが固定される。したがって finite-$\rho$ overlap subtraction は、LP、$B_\gamma$、log family のいずれか1つへ独立に適用してはならない。


## 79. Phase 61：finite triangle joint-soft ownership

Phase 60 の spatial density $G(R,S,v)$ を、実際の soft simplex、

$$
R+S\leq T
$$

で積分する。$R=qx$, $S=q(1-x)$ と変換すると、QEDCalc は $x$ 積分と $q$ 積分を解析的に閉じる。finite triangle normalization を $N(T,v)$ とすると、
...
$$
\lim_{T\to\infty}\frac{T[1-N(T,v)]}{\ln T}=2v
$$

および、

$$
\lim_{T\to\infty}\left(T[1-N(T,v)]-2v\ln T\right)=v(1-2\ln v)
$$

を exact に再生成する。physical cutoff $T=1/(\rho U)$ では、

$$
\lim_{\rho\to0}\frac{1-N(1/(\rho U),v)}{\rho\ln(1/\rho)}=2Uv
$$

なので、この truncation は regulator limit で消える。

API：

```python
corner_joint_soft_triangle_audit()
corner_phase61_joint_soft_triangle_audit()
```

## 80. Phase 62：Eq. (28) shift ownership

QEDCalc の raw shift は、

$$
p'-k\longrightarrow(1-uv)p'-u(1-v)p''-k
$$

である。printed Eq. (28) の対応係数は $-v(1-u)$ なので、

$$
\boxed{[-u(1-v)]-[-v(1-u)]=v-u}
$$

となる。

`corner_hard_primary_result()` は printed-Eq. (28) hard checkpoint、`corner_shift_correction_result()` はその hard checkpoint を shift-consistent result へ移す解析 correction である。current finite-$\rho$ raw bridge は最初から shift-consistent なので、この correction は current raw kernel へ追加しない。

API：

```python
corner_eq28_shift_ownership_audit()
corner_phase62_eq28_shift_ownership_audit()
```
...
`corner_phase71_cancellation_first_overlap_measure_audit()` は三角領域と1次元領域の `a_d=u*r` 変換について Jacobian と上限境界を exact に監査する。

`corner_cancellation_first_overlap_qmc(rho_value, power=..., seed=..., replicates=...)` は Phase 70 の5 sector を専用 map で評価する。三角 sector は `a_l=y` を保持したまま `r` を `0` から `(1-y)/u` まで対数 sampling し、line sector は `r` を `0` から `1/u` まで対数 sampling する。physical `u du` と `da_d=u dr` を明示的に1回だけ掛ける。

不確かさは独立 scrambled-Sobol replicate の assembled value の scatter から算出する。これは finite-rho numerical diagnostic であり、archived corrected coefficient は入力に使わない。

実行：`run_phase71_corner_cancellation_overlap_qmc.bat`

検証：`run_v078_validation.bat`

## v0.82.0：Phase 75 retained-photon route closure

`corner_retained_photon_residual_kernel()` は Phase 69 の `k2_cancel_quotient` をそのまま使い、`P D^2 L'` の n=4 family 上に有限 photon-mass residual を再構成する。Phase 75 の pre/post-cancellation sector comparison により、Gaussian helper は denominator continuation を既に所有しているため、追加の `(-1)^n` 比を掛けると符号を二重反転することが分かった。現在の cancellation coefficients は `D/E=-1`, `-(1/2)k^2/P=+1/2-rho^2/(2P)` をそのまま用いる。

## v0.83.0：Phase 76 soft finite ownership

`corner_phase76_soft_finite_ownership_audit()` は、安定化 numerical route の log-subtracted 極限が full finite ではなく hard remainder であることを明示します。
...
です。`corner_phase76_full_finite_qmc()` は Phase 75 corrected rational route、retained-photon residual、Phase 52 direct-log route から数値的 `H_fin` を作り、解析的 `C_soft` を一度だけ加えます。

## v0.84.0：Phase 77 corner end-to-end checkpoint

`corner_phase77_end_to_end_checkpoint()` は corner 図について、sector 経路、soft/hard matching 経路、closed-form checkpoint、self-energy insertion pair との IR-log cancellation を一括監査する。すべての解析 residual は exact に0である。

`corner_phase77_numerical_checkpoint(rho, ...)` は stabilized finite-$\rho$ QMC を解析 closure と比較する独立診断であり、解析 checkpoint を数値 kernel の入力には使わない。


## 78. v0.85.0：crossed ladder end-to-end closure checkpoint

`crossed_phase78_end_to_end_checkpoint()` は crossed ladder の高速 release checkpoint である。Breit projector の F1/F2 正規化、endpoint cutoff-log cancellation、half-sector + endpoint-sector の最終解析合成を exact residual 0 で検査する。raw q-kernel から automatic Hermite/canonical kernel までの重い全再生成は既存の専用 phase に残す。Karplus--Kroll 旧値との差 1/32 は歴史的 provenance 課題として分離し、発生箇所が未特定であることを明示する。

## v0.86.0 / Phase 79: vacuum polarization end-to-end closure

`vp_phase79_end_to_end_checkpoint()` は、既存の raw subloop 導出後の物理 invariant を一本化して監査する。D次元 transverse residual、$\Pi_R(0)$、D->4 kernel、外側 magnetic insertion、z-kernel、primitive derivative、endpoint、最終解析値の residual をすべて返す。raw LaTeX/topology parse は Phase 21 の独立テストに保持する。

## Phase 81: ordinary ladder end-to-end checkpoint

`examples/phase81_ordinary_ladder_end_to_end_checkpoint.py` は corrected spin-sum projector の 72 項を symmetry-canonical 40 target にまとめ、`ladder_corrected_40target_12basis_symbolic_reduction.csv` を用いて 12 master basis へ合成する。

監査項目：

- canonical target 数 = 40
- terminal basis 数 = 12
- physical master sum の leading $1/z$ residual = 0
- bare Laurent pole = $-3/(4\delta)$
- bare finite part = $107/48+\pi^2/18$
- one-loop subtraction finite part = 2
- renormalized result = $11/48+\pi^2/18$

full 12-master evaluation は高コストなので example 側で一度実行し、pytest 側では reduction size、z-pole cancellation、subtraction、renormalized closed form の軽量 invariant を監査する。


## Phase 83 — 2ループ完成版 regression checkpoint

`examples/phase83_two_loop_completion_regression_stdlib.py` は科学計算パッケージを import せず、Phase 77–82 の release artifacts と reduction data を監査する。

固定基底は

$$
\left\{
1,\;\pi^2,\;\zeta(3),\;\pi^2\ln2,\;\ln(1/\rho)
\right\}
$$
...
となる。最後の `0` は corner pair と self-energy insertion pair の IR logarithm が完全に相殺することを表す。

baseline は `data/two_loop_v090_baseline.json` に保存する。将来3ループ以上の機能を変更した後も `run_v090_validation.bat` を実行することで、2ループ完成状態を regression 検査できる。

SymPy が存在する場合は `phase83_two_loop_extended_scientific_regression.py` も自動実行され、corner、crossed ladder、vacuum polarization、self-energy の解析 residual を再評価する。ordinary ladder の重い full master reconstruction は専用 Phase 81 経路に保持し、通常の Phase 83 scientific regression では二重実行しない。
